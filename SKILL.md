---
name: luiwwi-software-architect
description: Usa esta skill cuando el usuario pida escribir, revisar, auditar, refactorizar u optimizar código de producción y quiera un estándar de calidad de nivel Principal/Distinguished Engineer en lugar de una solución superficial. Aplica en tareas como: generar código nuevo (backend PHP/Laravel, frontend TypeScript/React, o cualquier otro lenguaje) que deba ser robusto, seguro y escalable desde el primer commit; auditar código existente en busca de anti-patrones, cuellos de botella algorítmicos, vulnerabilidades de seguridad (inyección SQL, XSS, manejo de secretos) o deuda técnica; refactorizar funciones monolíticas, código acoplado o mal tipado hacia una arquitectura limpia (SOLID, Clean/Hexagonal Architecture); diseñar la topología de una solución nueva antes de implementarla; o validar que una entrega cumpla un checklist estricto de Definition of Done (manejo de errores, ausencia de placeholders, cobertura de edge cases, ausencia de N+1, complejidad ciclomática controlada). También trigger cuando el usuario mencione explícitamente "revisión de código de nivel senior/principal", "code review exhaustivo", "refactor con mejores prácticas", o pida que el código generado sea "listo para producción". Incluye guías específicas para el stack de Laravel/PHP y TypeScript/React, con extensiones para dominio de cumplimiento fiscal mexicano (CFDI 4.0/SAT) cuando aplique. No usar para snippets triviales, prototipos desechables o código exploratorio donde la velocidad prima sobre el rigor arquitectónico.
---
